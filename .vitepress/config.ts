...
async function config() {
  return {
    ...
    srcExclude:
      process.env.NODE_ENV === "development" ? [] : ["posts/**/$draft*.md"],
    // `rewrites` 规则会用 `path-to-regexp` 解析；新版本对 pattern 语法更严格，
    // 这里用函数方式做纯字符串替换，移除文件名里的 `$pin ` 前缀。
    rewrites: (id: string) =>
      id.replace(/^posts\/([^/]+)\/\$pin\s*/, "posts/$1/"),
    lang: "en-US",
